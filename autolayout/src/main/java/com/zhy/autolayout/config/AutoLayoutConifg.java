...
public class AutoLayoutConifg
{
    ...
    public void init(Context context)
    {
        ...
        /**
         * 判断manifest中的配置的屏幕尺寸比例是否和实际的屏幕尺寸比例相同，
         * 如果不同就在不改变配置宽度的情况下改变配置的高度，
         * 使得配置的屏幕尺寸比例和实际的屏幕尺寸比例相同
         */
        //实际的屏幕尺寸比例
        float phoneScale = mScreenHeight / (mScreenWidth * 1.0f);
        //manifest中配置的屏幕尺寸比例
        float manifestScale = mDesignHeight / (mDesignWidth * 1.0f);

        if (phoneScale != manifestScale) {
            mDesignHeight = (int)(mDesignWidth * phoneScale);
        }
